...
fs.readdir('./komutlar/', (err, files) => {//
    ...
    log(`════════════════════════════════════════      
    Toplam ${files.length} adet komut yükleniyor.
════════════════════════════════════════`);//
    files.forEach(f => {//
        let props = require(`./komutlar/${f}`);//
        log(`[BASARILI] ■ ${props.help.name} Komutu Yüklendi.`);//
...
//════════════════════════════════════════════════════════════════════════════════
// BOT SESE SOKMA KANAL ID 
client.on('ready', ()=>{
    client.channels.cache.get('849085315921739796').join() 
    })
...
client.on("message", message => {
    ...
    return message.channel.send(`${message.author}, aleykumselam.`)
});

client.on("message", message => {
    if(message.content.toLowerCase() == "sa") 
    return message.channel.send(`${message.author}, Aleykum Selam.`)
});

client.on("message", message => {
    if(message.content.toLowerCase() == "olimpos") 
    return message.channel.send(`${message.author}, KRALLL <3`)
});
//════════════════════════════════════════════════════════════════════════════════
// AFK SİSTEMİ
client.on("message" , async msg => {
 ...
 if(afk){
  ...
  if(msg.author.id === kisi){
       ...
       msg.channel.send(new Discord.MessageEmbed().setColor('BLACK').setDescription(`<@${kisi}> artık AFK değil !`))
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('848644334639382562');
  }, ms(sürejail));
}
})
//════════════════════════════════════════════════════════════════════════════════
client.on('guildMemberAdd', async(member) => {
let mute = member.guild.roles.cache.find(r => r.name === "848698295671980032");
...
db.delete(`muteli_${member.guild.id + member.id}`)
    ...
    member.roles.remove('848698295671980032');
  }, ms(süre));
}
})
//════════════════════════════════════════════════════════════════════════════════
...
data.set(`${member.guild.id}.jail.${kişi.id}.roles.${r.id}`, r.id )})
  ...
  .setColor(`#0x800d0d`)
  .setDescription(`Kullanıcı otomatik olarak jaile atıldı!`)
  .setTimestamp()
    member.send(wasted)
}
})
...
client.on("message", async msg => {
 ...
 const i = await db.fetch(`kufur_${msg.guild.id}`)
    if (i == "acik") {
        const kufur = ["amk", "aq", "oç", "sik", "çük", "yarak", "amcık", "göt", "ananı", "amq", "orospu", "orspu", "oruspu", "anani", "amcik", "sıkerım", "sıkerim", "sikerım", "yarram", "amık", "kahpe", "piç", "amsk", "seks", "sikiş", "yarrak", "gavat", "orospuçocuğu", "orospucocugu", "ananısikim", "gahpe", "kahpe", "ibne", "ipne", "pezevenk", "puşt", "fuck", "bitch", "bastard", "asshole"];
